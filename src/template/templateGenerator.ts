import { TemplateGenerationConfig } from './types';

/**
 * Generates the header comment for the mago.toml file
 * Includes project name if available
 */
export function generateTomlHeader(projectName?: string): string {
    const headerLines = ['# Generated by Mago VS Code extension'];
    
    if (projectName) {
        headerLines.push(`# Project: ${projectName}`);
    }
...
/**
 * Generates the complete mago.toml template content
 * Includes all necessary sections with comprehensive documentation
 */
export function generateMagoTomlTemplate(phpVersion: string): string {
    return `# Welcome to Mago!
...
/**
 * Generates the complete mago.toml file content
 * Combines header and template with proper formatting
 */
export function generateCompleteTomlContent(
    projectName?: string, 
    phpVersion: string = '8.4.0'
): string {
    const header = generateTomlHeader(projectName);
    const template = generateMagoTomlTemplate(phpVersion);
